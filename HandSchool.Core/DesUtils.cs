...
namespace HandSchool
{
    static class DesUtils
    {
        public static string StrEnc(string data, string firstKey, string secondKey, string thirdKey)
        {
            ...
            var encData = "";
            byte[][] firstKeyBt = Array.Empty<byte[]>(), secondKeyBt = Array.Empty<byte[]>(), thirdKeyBt = Array.Empty<byte[]>();
            ...
            if (leng > 0)
            {
                ...
                else
                {
                    ...
                    {
                        var tempData = data.SubStr(i * 4 + 0, i * 4 + 4);
                    ...
                    if (remainder > 0)
                    {
                        var remainderData = data.SubStr(iterator * 4 + 0, leng);
        ...
        */
        public static string StrDec(string data, string firstKey, string secondKey, string thirdKey)
        {
            ...
            var iterator = leng / 16;
            var i = 0;
            for (i = 0; i < iterator; i++)
            {
                var tempData = data.SubStr(i * 16 + 0, i * 16 + 16);
                var strByte = HexToBt64(tempData);
                var intByte = new byte[64];
                var j = 0;
                for (j = 0; j < 64; j++)
                {
                    intByte[j] = (byte)int.Parse(strByte.SubStr(j, j + 1));
                }
                byte[] decByte = Array.Empty<byte>();
                if (firstKey != null && firstKey != "" && secondKey != null && secondKey != "" && thirdKey != null && thirdKey != "")
                {
                    byte[] tempBt;
                    int x, y, z;
        ...
        */
        private static byte[][] GetKeyBytes(string key)
        {
            ...
            for (i = 0; i < iterator; i++)
            {
                keyBytes[i] = StrToBt(key.SubStr(i * 4 + 0, i * 4 + 4));
            }
            if (remainder > 0)
            {
                keyBytes[i] = StrToBt(key.SubStr(i * 4 + 0, leng));
            }
            return keyBytes;
        }
        ...
        */
        ...
        private static string HexToBt64(string hex)
        {
            ...
            for (var i = 0; i < 16; i++)
            {
                binary += HexToBt4(hex.SubStr(i, i + 1));
            }
            return binary;
        }
        ...
        */
        ...
        {
            var xorByte = new byte[byteOne.Length];
       ...
       new byte[]{2, 1, 14, 7, 4, 10, 8, 13, 15, 12, 9, 0, 3, 5, 6, 11 } };
            ...
            for (var m = 0; m < 8; m++)
            {
                ...
                sBoxByte[m * 4 + 0] = (byte)int.Parse(binary.SubStr(0, 1));
                sBoxByte[m * 4 + 1] = (byte)int.Parse(binary.SubStr(1, 2));
                sBoxByte[m * 4 + 2] = (byte)int.Parse(binary.SubStr(2, 3));
                sBoxByte[m * 4 + 3] = (byte)int.Parse(binary.SubStr(3, 4));
            }
            return sBoxByte;
        }
        ...
        private static string GetBoxBinary(byte i)
        {
            var binary = "";
            switch (i)
            {
        ...
        */
        ...
        private static string SubStr(this string the, int startIndex, int endIndex)
        {
            return the.Substring(startIndex, endIndex - startIndex);
        }
    }
}
